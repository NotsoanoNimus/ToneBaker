...
public static class WaveGenerator
{
    ...
    private const int AttenuationCycles = 2;
    ...
    /// <see cref="PCMSample"/>
    /// <see cref="AudioFormat"/>
    public static List<PCMSample> CreateNewWave(
        AudioFormat audioFormat,
        WaveType wavePattern,
        double amplitudePerc,
        double durationSec,
        double frequencyHz)
    {
        ...
        var samplesPerWaveCycle = audioFormat.SampleRate / frequencyHz;

        // Total amount of samples for the wave duration in its entirety.
        var totalSamples = durationSec * audioFormat.SampleRate;
        ...
        var extraSamples = samplesPerWaveCycle - (totalSamples % samplesPerWaveCycle);
        var waveAttenuationSamplesCount = (samplesPerWaveCycle * AttenuationCycles);
        ...
        var peakAmplitude = audioFormat.GetPeakAmplitude(amplitudePerc);
        ...
        Func<int, double> sampleCalculation = wavePattern switch
        {
            ...
            WaveType.Sawtooth => currSample =>
                ((peakAmplitude * ((2 * currSample / samplesPerWaveCycle) + 1)) % (peakAmplitude * 2)) -
                peakAmplitude,

            // f(x) = x, when 0 <= x < pi ;; f(x) = x-2pi, when pi <= x =< 2pi
            WaveType.Square => currSample =>
                (currSample % samplesPerWaveCycle) < (samplesPerWaveCycle / 2)
                    ? peakAmplitude
                    : 0 - peakAmplitude,
            ...
            WaveType.WhiteNoise => currSample =>
                (prng.NextDouble() * (audioFormat.MaxSampleValue - audioFormat.MinSampleValue)) +
                audioFormat.MinSampleValue,

            // If no other enum matched, throw exception
            _ => throw new Exception("CreateNewWave: Invalid wave pattern selection: " + wavePattern)
        };

        for (var currentSample = 0; currentSample < totalSamples + extraSamples; currentSample++)
        {
            var sampleValue = sampleCalculation(currentSample);
            ...
            if (currentSample < waveAttenuationSamplesCount)
            {
                ...
                sampleValue *= currentSample / waveAttenuationSamplesCount;
            }
            else if (currentSample > ((totalSamples + extraSamples) - waveAttenuationSamplesCount))
            {
                ...
                sampleValue *= (totalSamples + extraSamples - currentSample) / waveAttenuationSamplesCount;
            }
