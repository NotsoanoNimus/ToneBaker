...
public sealed class AudioFormat
{
    ...
    public AudioFormat(int sampleRate, int bitsPerSample, int channelCount)
    {
        if (sampleRate <= 0)
        {
            throw new ArgumentException("The sample rate must be a positive integer.");
        }

        if (bitsPerSample <= 0)
        {
            throw new ArgumentException("Bits-per-sample must be a positive integer.");
        }
        ...
        this.BytesPerSample = (bitsPerSample / 8) % 4;
    ...
    {
        var atPercentage = Math.Max(Math.Abs(forPercentage), 100.0);
...
public sealed class PCMSample
{
    ...
    public AudioFormat SampleFormat { get; }
    ...
    public PCMSample(AudioFormat sampleFormat, int sampleValue)
    {
        ...
        var discardClipping = sampleValue; //don't care about the ref, just can't use a `this`
    ...
    private void ClipSample(ref int sampleValue)
    {
        if (sampleValue != 0)
        {
    ...
    private void SetAllChannelValues(ref int sampleValue)
    {
        for (var r = 0; r < this.SampleFormat.ChannelCount; r++)
        {
    ...
    {
        this.ClipSample(ref sampleValue);
        for (var r = 0; r < this.SampleFormat.BytesPerSample; r++)
        {
    ...
    private int GetChannelValue(in int channelNumber)
    {
        try
        {
            ...
            var dwordSampleValueInChanel = new byte[] { 0, 0, 0, 0 };
            for (var i = 0; i < this.SampleFormat.BytesPerSample; i++)
            {
                dwordSampleValueInChanel[i] = this.Sample[i + (channelNumber * this.SampleFormat.BytesPerSample)];
            }

            // If for some reason, the system is big-endian, then reverse the byte array.
            if (!BitConverter.IsLittleEndian)
            {
                Array.Reverse(dwordSampleValueInChanel);
            }
            ...
            {
                for (var i = this.SampleFormat.BytesPerSample; i < dwordSampleValueInChanel.Length; i++)
                {
                    dwordSampleValueInChanel[i] ^= 0xFF;
                }
            }

            // Using ToInt32 here.
            return BitConverter.ToInt32(dwordSampleValueInChanel, 0);
        }
        catch
        {
            return 0;
